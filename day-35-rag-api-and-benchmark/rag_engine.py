...
class RAGEngine:
    ...
    def load(self):
        ...
        self.index=faiss.read_index(str(self.index_dir/"semantic.faiss"))
        self.vectorizer=joblib.load(self.index_dir/"tfidf_vectorizer.joblib")
        self.matrix=joblib.load(self.index_dir/"tfidf_matrix.joblib")
        self.chunks=json.loads((self.index_dir/"metadata.json").read_text(encoding="utf-8"))

    @staticmethod
    def norm(values):
        values=np.asarray(values,dtype="float32")
        if values.max()==values.min(): return np.zeros_like(values)
    ...
    def retrieve(self,query,top_k=4):
        ...
        s=self.norm(semantic); k=self.norm(keyword); hybrid=.65*s+.35*k
        ids=np.argsort(-hybrid)[:max(8,top_k*2)]
        candidates=[]
        for i in ids:
            x=dict(self.chunks[int(i)]); x["hybrid_score"]=float(hybrid[i])
            candidates.append(x)
