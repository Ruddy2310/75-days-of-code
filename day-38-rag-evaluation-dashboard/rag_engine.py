...
class RAG:
    def __init__(self,index_dir="index"):
        ...
        self.cache=Cache(); self.index=None; self.vectorizer=None; self.matrix=None; self.chunks=[]
    def load(self):
    ...
    def rewrite(self,message,history):
        ...
        if words & {"it","this","that","they","them","these","those","its"} or len(message.split())<6:
    ...
    def answer(self,message,history=None,use_llm=False):
        history=history or []; query=self.rewrite(message,history); results=self.retrieve(query)
        ...
        if use_llm:
            try:
                r=requests.post("http://localhost:11434/api/generate",
                    ...
                    "Answer only from the evidence. If insufficient, say so.\n"
                    f"Question: {message}\n\nEvidence:\n{context}","stream":False},timeout=120)
                r.raise_for_status(); answer=r.json()["response"].strip()
            except Exception as e: answer+=f"\n\n[Ollama unavailable: {e}]"
        return {"answer":answer,"retrieval_query":query,
